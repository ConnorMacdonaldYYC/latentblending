...
class DiffusersHolder:
    ...
    def init_types(self):
        ...
        ), "No valid diffusers pipeline found."
    ...
    @torch.no_grad()
    def latent2image(self, latents: torch.FloatTensor, output_type="pil"):
        r"""
        ...
        if needs_upcasting:
            ...
            latents = latents.to(
                next(iter(self.pipe.vae.post_quant_conv.parameters())).dtype
            )
    ...
    def prepare_mixing(self, mixing_coeffs, list_latents_mixing):
        if type(mixing_coeffs) == float:
            ...
            assert (
                len(mixing_coeffs) == self.num_inference_steps
            ), f"len(mixing_coeffs) {len(mixing_coeffs)} != self.num_inference_steps {self.num_inference_steps}"
            list_mixing_coeffs = mixing_coeffs
        else:
            raise ValueError(
                "mixing_coeffs should be float or list with len=num_inference_steps"
            )
        if np.sum(list_mixing_coeffs) > 0:
            assert (
                len(list_latents_mixing) == self.num_inference_steps
            ), f"len(list_latents_mixing) {len(list_latents_mixing)} != self.num_inference_steps {self.num_inference_steps}"
    ...
    def run_diffusion(
        ...
        return self.run_diffusion_sd_xl(
            text_embeddings,
            latents_start,
            idx_start,
            list_latents_mixing,
    ...
    def run_diffusion_sd_xl(
        ...
        timesteps, num_inference_steps = retrieve_timesteps(
            self.pipe.scheduler, self.num_inference_steps, device, timesteps
        )
        ...
        if ip_adapter_image is not None:
            output_hidden_state = (
                False
                if isinstance(self.pipe.unet.encoder_hid_proj, ImageProjection)
                else True
            )
        ...
        num_warmup_steps = max(
            len(timesteps) - num_inference_steps * self.pipe.scheduler.order, 0
        )
        ...
        for i, t in enumerate(timesteps):
            ...
            if i > 0 and list_mixing_coeffs[i] > 0:
                ...
                latents = interpolate_spherical(
                    latents, latents_mixtarget, list_mixing_coeffs[i]
                )
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        img_refx = self.run_diffusion_sd_xl(
